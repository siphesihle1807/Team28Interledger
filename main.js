...
import {
  ...
  (async () => {
    const client = await createAuthenticatedClient({
      walletAddressUrl: "https://ilp.interledger-test.dev/cedeee60", // Make sure the wallet address starts with https:// (not $), and has no trailing slashes
      privateKey: "private.key",
      keyId: "4dde07f1-02fb-4a49-a464-d7a7db121e83",
    });
  
    const sendingWalletAddress = await client.walletAddress.get({
      url: "https://ilp.interledger-test.dev/cedeee60", // Make sure the wallet address starts with https:// (not $)
    });
    const receivingWalletAddress = await client.walletAddress.get({
      url: "https://ilp.interledger-test.dev/siphe", // Make sure the wallet address starts with https:// (not $)
    });
